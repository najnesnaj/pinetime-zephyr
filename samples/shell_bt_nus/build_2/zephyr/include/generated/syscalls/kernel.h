...
static inline int k_pipe_put(struct k_pipe * pipe, void * data, size_t bytes_to_write, size_t * bytes_written, size_t min_xfer, k_timeout_t timeout)
{
	...
	if (z_syscall_trap()) {
		...
		return (int) arch_syscall_invoke6(parm0.x, parm1.x, parm2.x, parm3.x, parm4.x, parm5.x, K_SYSCALL_K_PIPE_PUT);
	}
#endif
	compiler_barrier();
...
static inline int k_pipe_get(struct k_pipe * pipe, void * data, size_t bytes_to_read, size_t * bytes_read, size_t min_xfer, k_timeout_t timeout)
{
	...
	if (z_syscall_trap()) {
		...
		union { uintptr_t x; size_t val; } parm4 = { .val = min_xfer };
		union { uintptr_t x; k_timeout_t val; } parm5 = { .val = timeout };
		return (int) arch_syscall_invoke6(parm0.x, parm1.x, parm2.x, parm3.x, parm4.x, parm5.x, K_SYSCALL_K_PIPE_GET);
	}
#endif
	compiler_barrier();
