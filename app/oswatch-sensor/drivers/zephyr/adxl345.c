			       ...
			       struct adxl345_sample *sample)
{
	...
	int rc = i2c_burst_read(data->i2c_master,
				...
				axis_data,
				6);
/*        int rc = i2c_reg_read_byte(data->i2c_master, data->i2c_addr,
                0x06, &axis_data[0]);
...
printk("part id reading\n");
	if (rc < 0 || dev_id != ADXL345_PART_ID) {
		LOG_ERR("this does not mean anything Read PART ID failed: 0x%x\n", rc);
//		return -ENODEV;
	}
for(int i = 0; i < 5; i ++)
{
	rc = i2c_reg_write_byte(data->i2c_master, data->i2c_addr,
		...
		//hx022 CNTL2, ADXL345_FIFO_STREAM_MODE);
		0x19, 0xBF);
	if (rc < 0) {
